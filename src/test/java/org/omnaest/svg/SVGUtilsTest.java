...
public class SVGUtilsTest
{
    ...
    @Test
    @Ignore
    public void testBoundedContextWithSlicesRender() throws IOException
    {
        SVGDrawer drawer = SVGUtils.getDrawer(900, 600)
                                   .withScreenDimensions(DisplayResolution._1280x800);
        BoundedArea boundedArea = drawer.newBoundedArea();
        ...
        {
            //
            List<BoundedArea> verticalSlices = boundedArea.asVerticalSlices(2, 3, 5);

            verticalSlices.forEach(slice -> slice.withScalingHeight(100)
                                                 .withScalingWidth(100)
                                                 .add(new SVGRectangle(0, 0, 100, 100)));
        }
        {
            List<BoundedArea> horizontalSlices = boundedArea.asHorizontalSlices(3, 2, 1);
            horizontalSlices.forEach(slice -> slice.withScalingHeight(100)
                                                   .withScalingWidth(100)
                                                   .add(new SVGRectangle(0, 0, 100, 100)));
        }

        drawer.renderAsResult()
              .writeToFile(new File("C:/Temp/svgSlicesPropoertionsTest.svg"));
    }
